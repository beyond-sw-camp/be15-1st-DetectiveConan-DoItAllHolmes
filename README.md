...
| 조원명 | REIVIEW |
| --- | --- |
| 김기홍 |비전공자로서 첫 프로젝트를 진행하다 보니 부족한 점이 많았습니다. 다행히 좋은 팀원들을 만나 많은 도움을 받으며 한걸음 성장할 수 있었습니다. 이론으로만 알고 있던 데이터베이스를 실제 프로젝트 수행을하며  쿼리문을 작성하고 구현하면서 이해하지 못했던 부분들을 조금 더 자세히 익힐 수 있었습니다. 짧은 기간이었지만, 훌륭한 팀원들과 함께 프로젝트를 수행할 수 있어 영광이었습니다. 다음 백엔드 프로젝트에서는 1.5인분을 해낼 수 있도록 매일 성장하며 노력하겠습니다.|
| 김태환 |DB를 활용한 프로젝트를 통해 많은 점을 얻은 것 같다. 쿼리문 작성 능력, 프로시저, 트리거 등 고급 sql문과 같은 것을 배우면서 sql에 대한 이해도 또한, 가질 수 있었다. 그리고 팀프로젝트였어서 팀원과 안 맞으면 걱정했지만, 한 분 한 분이 너무 좋으신 분이고 의견 또한 잘 맞아서 막힘없이 결과를 만들 수 있었다. 감사합니다!|
...
| 이기연 |처음으로 Use Case 작성부터 논리 및 물리 ERD 설계, 그리고 실제 구축까지 경험해보았습니다. 처음 해보는 과정이라 긴장도 되었고, 실수할까 걱정도 많았지만, 다행히 좋은 팀원을 만나 함께 해결해 나갈 수 있었습니다. 특히 ERD를 설계할 때, 데이터 간의 관계를 명확히 정의하는 것이 중요하다는 것을 배웠고, 논리 모델에서 물리 모델로 변환하는 과정에서 세세한 부분까지 신경 써야 한다는 점을 깨달았습니다. 또한, 실무와 비슷한 경험을 하면서 단순히 기능을 구현하는 것이 아니라, 체계적으로 설계하는 것이 얼마나 중요한지 몸소 느낄 수 있었습니다. 팀원들과 적극적으로 소통하며 어려운 부분을 함께 고민하고 해결했던 덕분에 처음의 긴장감을 잘 극복할 수 있었고, 프로젝트를 성공적으로 마무리할 수 있었습니다. 이번 경험을 통해 설계의 중요성을 배우고, 팀워크의 힘을 다시 한번 실감했습니다!!|
| 이채은 |이번 프로젝트를 통해 굉장히 많은 것을 얻은 것 같다. 첫 번째로는 내가 어디 부분이 부족한 부분을 연습하고 공부할 수 있는 시간이었다. SQL 문을 제대로 쓰지 못 했었는데 지금은 일상 생활에서도 가끔 "이건 SQL문으로 이거야" 라고 생각이 들 때가 있을 정도로 많이 익숙해진 것 같다.두번째는 앞으로의 프로젝트에서 어떤 부분을 고려해야 될 지에 대해서 알 수 있던 시간이었다. 재밌는 팀원들을 만나서 스트레스 안 받고 웃으면서 프로젝트 했던 것 같다! 행복한 시간이었다!|
| 최지혜 |프로젝트는 처음이라 어리바리했는데 팀원분들의 프로젝트 경험들로 프로젝트의 방향을 잡아나갈 수 있었습니다. 프로젝트를 통해 3주 동안 학습한 내용들을 적용해 보며 부족한 부분을 깨닫고 배워나갈 수 있는  좋은 기회가 되었습니다. 주제를 정하는 것부터 어떤 기능들을 넣을지 고민하고 구현해 내는 전 과정들이 새롭고 즐거웠습니다. 주제 정할 때부터 프로젝트 끝날 때까지 팀원들 모두 열정 가득하며 적극적인 참여로 더 즐겁게 프로젝트를 해 낼 수 있었습니다. 질문도 많이 하고 이런 저런 의견도 많이 냈는데 잘 알려주시고 잘 들어주셔서  감사했습니다.|
